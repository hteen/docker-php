...
RUN apt-get update && apt-get install -y autoconf file g++ gcc libc-dev make pkg-config re2c ca-certificates curl libedit2 libsqlite3-0 libxml2 git libcurl4-gnutls-dev libcurl4-openssl-dev freetds-dev libbz2-dev libc-client-dev libenchant-dev libfreetype6-dev libgmp3-dev libicu-dev 

RUN apt-get install -y libjpeg62-turbo-dev libkrb5-dev libmcrypt-dev libmemcached-dev libpng12-dev libpq-dev libpspell-dev libsasl2-dev libsnmp-dev libssl-dev libtidy-dev libxml2-dev libxpm-dev libxslt1-dev zlib1g-dev libcurl4-openssl-dev libedit-dev libsqlite3-dev xz-utils

ENV PHP_INI_DIR /etc/php
ENV PHP_SCAN_INI_DIR /usr/local/etc/php
RUN mkdir -p $PHP_SCAN_INI_DIR/conf.d
...
RUN curl -fSL "http://php.net/get/$PHP_FILENAME/from/this/mirror" -o "$PHP_FILENAME" \
    ...
    && ./configure \
        ...
        --with-config-file-scan-dir="$PHP_SCAN_INI_DIR/conf.d" \
